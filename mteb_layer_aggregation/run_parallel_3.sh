...
CUDA_VISIBLE_DEVICES=2 python scripts/run_mteb_eval.py \
    ...
    --lambda-values 1.0 \
    --methods weighted \
    --output-dir ./aggregation_results_bert_mean_fisher1 \
    --max-samples 10000000 --batch-size 512 \
    --per-dataset-similarity-metric CKA \
    --per-dataset-hessian \
    --hessian-cache-dir "./hessian_cache5_bert" \
    --similarity-cache-dir "./similarity_cache3" \
    --use-embedding-cache --use-quality-cache > bert_mean9_output.log 2>&1 &
...
CUDA_VISIBLE_DEVICES=3 python scripts/run_mteb_eval.py \
    ...
    --lambda-values  1.0 \
    --methods weighted \
    --output-dir ./aggregation_results_simcse_mean_fisher1 \
    --max-samples 10000000 --batch-size 512 \
    --per-dataset-similarity-metric CKA \
    --per-dataset-hessian \
    --hessian-cache-dir "./hessian_cache5_sim_cse" \
    --similarity-cache-dir "./similarity_cache3" \
    --use-embedding-cache --use-quality-cache > simcse_mean9_output.log 2>&1 &
